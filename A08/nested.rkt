...
;; (nested-cleanup nlox)  consumes a 
;;   nlox (nested-listof X) and removes any empty
;;   list anywhere in the list
;; nested-cleanup: (nested-listof X) -> (Anyof
;;                   List Bool)
;; Examples:
(check-expect (nested-cleanup '(() 1 2 (3 4) 5 (6 7 (8)) 9 () (((hot-dog)))))
'(1 2 (3 4) 5 (6 7 (8)) 9 (((hot-dog)))))
(check-expect (nested-cleanup '()) false)

(define (nested-cleanup nlox)
  (cond
    [(empty? nlox) false]
    [(not (list? (first nlox))) (cond
                                  [(false? (nested-cleanup (rest nlox)))
                                   (cons (first nlox) empty)]
                                  [else (cons (first nlox) (nested-cleanup (rest nlox)))])]
    [(and (false? (nested-cleanup (first nlox)))
          (false? (nested-cleanup (rest nlox)))) false]
    [(false? (nested-cleanup (first nlox))) (nested-cleanup (rest nlox))]
    [(false? (nested-cleanup (rest nlox))) (cons (nested-cleanup (first nlox)) empty)]
    [else (cons (nested-cleanup (first nlox)) (nested-cleanup (rest nlox)))]))

;; Tests:
(check-expect (nested-cleanup '(0 () 1 () () 2)) '(0 1 2))
(check-expect (nested-cleanup '(0 (()) 1 (() () 3) () 2)) (list 0 1 (list 3) 2))
(check-expect (nested-cleanup '(0 (()) 1 (() () 3) (1(2(3))) ((()2)) 2)) (list
 0
 1
 (list 3)
 ...
 (list (list 2))
 2))
(check-expect (nested-cleanup '("string" (()()) "another string" (("more string" () (()))) ))
              (list
 "string"
 "another string"
 (list (list "more string"))))